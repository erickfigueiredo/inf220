<template>
  <div class="flex justify-center items-center h-screen bg-gray-800 px-6">
    <div class="p-6 max-w-sm w-full bg-white shadow-md rounded-md">
      ...
      <form class="mt-4" @submit.prevent="retrieve">
        ...
        <div class="mt-6">
          <button
            :disabled="disabled"
            type="submit"
            class="inline-flex justify-center items-center transition duration-150 bg-red-600 hover:bg-red-800 text-white font-bold py-2 px-4 border-b-4 border-red-800 focus:outline-none rounded w-full"
          >
            <svg
...
export default defineComponent({
  ...
  methods: {
    async retrieve() {
      ...
      const result = await Login.recoverPassword(this.email);
      if (result.success) {
        this.type = "success";
