...
const createValidate = () => {
    return Joi.object().keys({
        ...
        business_name: Joi.string().regex(/^[A-Za-záàâãéèêíïóôõöúçñÁÀÂÃÉÈÍÏÓÔÕÖÚÇÑ ]+$/).min(1).max(150).required().messages({
            'string.pattern.base': 'Empresa deve ter somente letras!',
            'string.empty': 'Empresa não pode ser nulo!',
        ...
        cnpj: Joi.string().regex(/^[0-9]{14}$/).required().messages({
            'string.empty': 'CNPJ não pode ser nulo!',
...
const updateValidate = () => {
    return Joi.object().keys({
        ...
        id: Joi.number().integer().min(1).required().required().messages({
            'number.integer': 'Id deve ser um interio!',
            'number.min': 'Id não pode ser menor que 1!',
            'number.empty': 'Id não pode ser vazio!',
            'any.required': 'Id é obrigatório!'
        }),
        business_name: Joi.string().regex(/^[A-Za-záàâãéèêíïóôõöúçñÁÀÂÃÉÈÍÏÓÔÕÖÚÇÑ ]+$/).min(1).max(150).messages({
            'string.pattern.base': 'Empresa deve ter somente letras!',
            'string.empty': 'Empresa não pode ser nulo!',
            'string.min': 'Empresa não pode ter menos de 1 caractere!',
            'string.max': 'Empresa não pode ter mais de 50 caracteres!',
